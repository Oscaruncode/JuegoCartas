...
export function CardComponent({ card, selected, onClick, disabled, faceDown, revealStats }: Props){
  if (faceDown) {
    return (
  ...
  {revealStats ? (
    <>
      ...
      <div className="flex justify-between">
        ...
        <span className="font-bold">{card.stats.experiencia}</span>
      </div>
      <div className="flex justify-between">
        <span>💣 Escándalos</span>
        <span className="font-bold text-red-300">{card.stats.escandalos}</span>
      </div>
    </>
  ) : (
    <>
